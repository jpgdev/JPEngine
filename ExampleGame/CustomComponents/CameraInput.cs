...
namespace ExampleGame.CustomComponents
{
    public class CameraInput : BaseComponent
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (Engine.Input.Keyboard.IsDown((Keys) Engine.Settings["W"].Value))
            {
                Transform.Position.Y -= _speed * deltaSinceLastUpdate;
            }

            if (Engine.Input.Keyboard.IsDown((Keys)Engine.Settings["S"].Value))
            {
                Transform.Position.Y += _speed * deltaSinceLastUpdate;
            }

            if (Engine.Input.Keyboard.IsDown((Keys)Engine.Settings["D"].Value))
            {
                Transform.Position.X += _speed * deltaSinceLastUpdate;
            }

            if (Engine.Input.Keyboard.IsDown((Keys)Engine.Settings["A"].Value))
            {
                Transform.Position.X -= _speed * deltaSinceLastUpdate;
            }
            ...
            if (Engine.Input.Keyboard.IsClicked((Keys)Engine.Settings["PageUp"].Value))
            {
                Transform.Scale *= 1.1f;
            }

            if (Engine.Input.Keyboard.IsClicked((Keys)Engine.Settings["PageDown"].Value))
            {
